...
from app.models import UserListPermission, ShoppingItem, User
...
async def test_full_user_workflow(client: TestClient, session: Session):
    ...
    # Create a user in the database for authentication
    hashed_password = hash_string_password("testpassword")
    db_user = User(name="Integration Test User", hashed_password=hashed_password)
    session.add(db_user)
    session.commit()
    session.refresh(db_user)

    # Get authentication headers
    headers = get_auth_headers(db_user)
    ...
    response = client.post("/listperm/", params={"user_id": db_user.id, "list_id": shopping_list["id"]}, headers=headers)
    assert response.status_code == 200
    ...
    response = client.post(f"/shoppinglist/{shopping_list['id']}/items", json=item1_data, headers=headers)
    assert response.status_code == 200
    item1 = response.json()
    ...
    response = client.post(f"/shoppinglist/{shopping_list['id']}/items", json=item2_data, headers=headers)
    assert response.status_code == 200
    item2 = response.json()
    ...
    # Retrieve user lists
    response = client.get(f"/user/{db_user.id}/lists", headers=headers)
    assert response.status_code == 200
...
async def test_cascade_delete_workflow(client: TestClient, session: Session):
    ...
    # Create a user in the database for authentication
    hashed_password = hash_string_password("testpassword")
    db_user = User(name="Cascade Test User", hashed_password=hashed_password)
    session.add(db_user)
    session.commit()
    session.refresh(db_user)
    ...
    permission = session.get(UserListPermission, (db_user.id, shopping_list["id"]))
