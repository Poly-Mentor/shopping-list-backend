...
async def test_create_list(session):
    ...
    created_list = await shoppinglist_service.create_list(list_create, user, session)

    # Verify the list was created correctly
    assert created_list.id is not None
    assert created_list.name == "New List"
    assert created_list.owner_id == user.id
...
async def test_update_list(session):
    ...
    list_update = ShoppingListCreate(name="Updated Name")
    updated_list = await shoppinglist_service.update_list(shopping_list.id, list_update, user, session)
...
async def test_update_list_not_found(session):
    ...
    list_update = ShoppingListCreate(name="Updated Name")
    with pytest.raises(HTTPException) as exc_info:
        await shoppinglist_service.update_list(999, list_update, user, session)
...
async def test_delete_list(session):
    ...
    result = await shoppinglist_service.delete_list(shopping_list.id, user, session)
...
async def test_delete_list_not_found(session):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await shoppinglist_service.delete_list(999, user, session)
...
async def test_get_items_from_list(session):
    ...
    items = await shoppinglist_service.get_items_from_list(shopping_list.id, user, session)
...
async def test_add_item(session):
    ...
    added_item = await shoppinglist_service.add_item(shopping_list.id, user, item_create, session)
